...
const programRegistrationSchema = z
  .object({
...
interface ProgramDates {
  tuesdayEvening: { date: Date; display: string; value: string };
  wednesdayAfternoon: { date: Date; display: string; value: string };
  wednesdayEvening: { date: Date; display: string; value: string };
}
...
const ProgramRegistrationModal = ({ isOpen, onClose, programType }: ProgramRegistrationModalProps) => {
  ...
  const onSubmit = async (data: ProgramRegistrationForm) => {
    ...
    try {
      const { error } = await supabase.functions.invoke("submit-program-registration", {
        body: {
          ...data,
          programType,
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <div className="flex items-center gap-2 mt-4">
            {[1, 2, 3, 4].map((s) => (
              <div key={s} className={cn("h-2 flex-1 rounded-full", s <= step ? "bg-brand-orange" : "bg-gray-200")} />
            ))}
          </div>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-6">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {birthDate ? format(birthDate, "PPP", { locale: nl }) : "Selecteer een datum"}
                    </Button>
                  </PopoverTrigger>
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <RadioGroup
                ...
                onValueChange={(value) => setValue("registrationType", value as "particulier" | "zakelijk")}
              >
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <div className="bg-brand-blue-light/20 p-4 rounded-lg space-y-2">
                <p className="text-sm text-brand-gray-medium">
